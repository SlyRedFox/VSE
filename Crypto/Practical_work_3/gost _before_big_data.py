...
def elliptic_points(a: int, b: int, x_range: tuple) -> list:
    ...
    for x in range(x_range[0], x_range[1] + 1):
        y2 = x ** 3 + a * x + b  # Вычисляем значение y^2 = x^3 + ax + b
        if y2 >= 0:  # Проверяем, что y^2 неотрицательно, так как нет действительных корней для отрицательных чисел
            y = int(y2 ** 0.5)  # Находим целую часть квадратного корня из y^2
