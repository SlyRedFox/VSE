...
# TODO: del, это будет из файла
print(f'Сообщение в виде бинарного кода: {final_binary_code}')
...
# в нашем случае длина блока равна 12 бит, берём их с конца, т.к. начало в случае
# необходимости можно дополнить незначащими нулями, а если добавим их в конец - это уже будет другое значение).
print(f'Берём данные бинарного кода {final_binary_code}, считаем с конца {binary_log} бит.')
...
print('Находим длину блока для ШифроТекса. По формуле двоичный логарифм с округлением вниз от n + 1: math.floor(math.log2(n)) + 1')
...
# Расшифрование
# Шаг 1, берём нашу бинарную последовательность и переводим в десятичное представление
decrypted_dec: int = int(binary_code_ci, 2)
print(f'Десятичное представление символа: {decrypted_dec}')
